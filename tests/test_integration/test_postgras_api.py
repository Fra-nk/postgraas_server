...
class TestPostgraasApi(unittest.TestCase):
    ...
    def test_create_docker_fails(self):
        ...
        created_db = json.loads(result.data)
        self.assertTrue(
            'let create fail' in created_db["msg"],
    ...
    def test_delete_postgres_instance_api(self):
        ...
        self.assertTrue(
            wait_success,
            'postgres did not come up within 10s (or unexpected docker image log output)'
        )
        ...
        self.assertTrue(
            'password authentication failed' in deleted_db['msg'],
        ...
        with patch.object(docker.models.containers.Container, 'remove', raise_apierror):
            ...
            self.assertEqual(deleted_db["status"], 'failed')
            self.assertTrue(
                'let remove fail' in deleted_db['msg'],
    ...
    def test_delete_docker_notfound(self):
        ...
        self.assertTrue(
            'deleted postgraas instance, but container was not found' in res['msg'],
    ...
    def test_driver_name_exists(self):
        ...
        self.assertRaises(
            ValueError, pid.create_postgres_instance, 'test_instance_name', db_credentials
        )
    ...
    def test_create_postgres_instance_name_exists(self):
        ...
        self.assertEqual(
            ...
            json.dumps(
                {
                    "msg":
                    "postgraas_instance_name already exists tests_postgraas_my_postgraas_twice"
                }
            ) + "\n"
        )
        self.delete_instance_by_name(db_credentials)
